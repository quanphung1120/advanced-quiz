...
type CollectionQuery struct {
	...
	withCollaborators *CollectionCollaboratorQuery
	withFlashcards    *FlashcardQuery
...
func (_q *CollectionQuery) QueryCollaborators() *CollectionCollaboratorQuery {
	query := (&CollectionCollaboratorClient{config: _q.config}).Query()
...
// QueryFlashcards chains the current query on the "flashcards" edge.
func (_q *CollectionQuery) QueryFlashcards() *FlashcardQuery {
...
func (_q *CollectionQuery) Clone() *CollectionQuery {
	...
	return &CollectionQuery{
		...
		withFlashcards:    _q.withFlashcards.Clone(),
...
func (_q *CollectionQuery) WithCollaborators(opts ...func(*CollectionCollaboratorQuery)) *CollectionQuery {
	query := (&CollectionCollaboratorClient{config: _q.config}).Query()
...
// WithFlashcards tells the query-builder to eager-load the nodes that are connected to
// the "flashcards" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *CollectionQuery) WithFlashcards(opts ...func(*FlashcardQuery)) *CollectionQuery {
	query := (&FlashcardClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withFlashcards = query
	return _q
}
...
func (_q *CollectionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Collection, error) {
	...
	if query := _q.withCollaborators; query != nil {
		if err := _q.loadCollaborators(ctx, query, nodes,
			func(n *Collection) { n.Edges.Collaborators = []*CollectionCollaborator{} },
			func(n *Collection, e *CollectionCollaborator) {
				n.Edges.Collaborators = append(n.Edges.Collaborators, e)
			}); err != nil {
			return nil, err
		}
	}
	if query := _q.withFlashcards; query != nil {
		if err := _q.loadFlashcards(ctx, query, nodes,
			func(n *Collection) { n.Edges.Flashcards = []*Flashcard{} },
			func(n *Collection, e *Flashcard) { n.Edges.Flashcards = append(n.Edges.Flashcards, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *CollectionQuery) loadCollaborators(ctx context.Context, query *CollectionCollaboratorQuery, nodes []*Collection, init func(*Collection), assign func(*Collection, *CollectionCollaborator)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(collectioncollaborator.FieldCollectionID)
	}
	query.Where(predicate.CollectionCollaborator(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(collection.CollaboratorsColumn), fks...))
	}))
...
func (_q *CollectionQuery) loadFlashcards(ctx context.Context, query *FlashcardQuery, nodes []*Collection, init func(*Collection), assign func(*Collection, *Flashcard)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Collection)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(flashcard.FieldCollectionID)
	}
	query.Where(predicate.Flashcard(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(collection.FlashcardsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
