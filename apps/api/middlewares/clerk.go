...
func ClerkAuthMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		fmt.Printf("[CLERK AUTH] Processing request: %s %s (Token length: %d)\n", c.Request.Method, c.Request.URL.Path, len(headerToken))
		...
		fmt.Printf("[CLERK AUTH] Authentication successful for %s %s (UserID: %s)\n", c.Request.Method, c.Request.URL.Path, claims.Subject)
