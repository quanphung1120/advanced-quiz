...
export function CollectionPage() {
  ...
  const readinessLabel = useMemo(() => {
    ...
    if (cardCount < 5) return "Almost ready";
    return "Ready";
  }, [cardCount]);
  ...
  if (collectionQuery.isPending || flashcardsQuery.isPending) {
    return (
      <div className="space-y-6 animate-pulse">
        <div className="h-5 w-40 rounded-sm bg-muted/30" />
        <div className="h-px bg-border" />
        <div className="h-40 rounded-sm bg-muted/30 border border-border" />
        <div className="h-px bg-border" />
        <div className="h-8 w-56 rounded-sm bg-muted/30" />
        <div className="h-64 rounded-sm bg-muted/30 border border-border" />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      ...
      {/* ── Tabs ── */}
      <TabsRoot defaultValue="overview" className="space-y-8">
        <TabsList>
          <TabsIndicator />
        ...
        <TabsPanel value="overview" className="space-y-0">
          <FlashcardStack
